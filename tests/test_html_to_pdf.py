...
class TestHTMLToPDFConverter:
    ...
    def test_convert_html_file_all_backends(self, backend, example_html_file, output_pdf_path):
        ...
        try:
            ...
            assert os.path.exists(output_file), f"Output PDF file not created for {backend} backend"
            
            # Check that file has content (not empty)
            assert os.path.getsize(output_file) > 0, f"Output PDF file is empty for {backend} backend"
    ...
    def test_full_conversion_workflow(self, example_html_file, test_data_dir):
        ...
        for backend in ["weasyprint", "xhtml2pdf", "reportlab"]:
            try:
                ...
                if result:
                    assert os.path.exists(output_file), f"Output file not created for {backend}"
                    assert os.path.getsize(output_file) > 0, f"Output file is empty for {backend}"
    ...
    def test_cleanup_generated_files(self, test_data_dir):
        ...
        for pdf_file in pdf_files:
            if any(test_name in pdf_file.name for test_name in [
                "example_output_", "string_test", "css_test", 
                "margin_test", "pagesize_test", "error_test", "integration_test_"
            ]):
                try:
                    pdf_file.unlink()
                    cleanup_count += 1
